...
async def get_ws_endpoints(device_id, user_id, proxy_url):
    ...
    async with ClientSession(connector=connector) as session:
        for attempt in range(5):
            try:
                async with session.post(f"{DIRECTOR_SERVER}/checkin", json=payload,
                                        headers=HEADERS, proxy=proxy_url) as resp:
                    if resp.status == 429:
    ...
    async def connect(self):
        ...
        while True:
            try:
                ...
                HEADERS["User-Agent"] = ua.random
                async with proxy_connect(self.uri, proxy=Proxy.from_url(self.proxy_url),
                                         ssl=ssl_ctx, extra_headers=HEADERS) as ws:
